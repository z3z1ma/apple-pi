...
- Direct Advisor consultation through the existing `Agent` tool.
...
- AC-002: Consultation context retains the active request and labels hypotheses and main-agent drafts as unverified without extension-defined packet ceilings.
- AC-003: `Agent` invokes the existing Advisor sub-agent in consultation mode without full transcript inheritance or a hand-authored summary.
...
- AC-006: Confirmed or refined findings are delivered only while implicated working state remains current; interruption cannot surprise-resume the main agent.
