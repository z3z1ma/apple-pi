...
- 2026-08-20: Adapted the specification reviewer template to `ledger-pi-review` with Ledger task/spec/decision context, explicit provenance and verifiability challenges, planning-readiness verdicts, and task-local review recording.
...
- WI-004 specification-review evidence: `skills/ledger-brainstorming/spec-document-reviewer-prompt.md` now names the real `ledger-pi-review` consumer, requires task/spec/active-record context, distinguishes semantic authority from current-code evidence, and returns planning readiness plus actionable findings for the task Review section.
