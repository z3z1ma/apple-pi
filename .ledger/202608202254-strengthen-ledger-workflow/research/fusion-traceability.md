...
| --- | --- | --- |
| `brainstorming/spec-document-reviewer-prompt.md` | Calibrated completeness/consistency/clarity/scope/YAGNI design review. | Read-only `pi_exec` specification-review reference under `ledger-brainstorming`; blocking semantic issues remain task blockers. |
...
| `writing-plans/plan-document-reviewer-prompt.md` | Detailed plan review against requirements, interfaces, ordering, tests, and buildability. | Bounded read-only `pi_exec` plan-review reference used by `ledger-writing-plans`, with explicit advisory/blocking results. |
...
## 2026-08-21 Ledger-first reconciliation

The operator added `z3z1ma/10x` at pinned commit `4616e5c07d6f9b82fb299ef18446280ab6f1e09d` as the durable-judgment substrate and required the incoming fused lifecycle skills to use the `ledger-` prefix. `research/ten-x-ledger-philosophy.md` is the source-backed concept mapping and `decisions/ledger-prefix-boundary.md` owns the corrected public topology.

| 10x source concept | Retained apple-pi behavior | Owner |
| --- | --- | --- |
| Impact over output; eliminate before adding | Smallest complete solution; exact trivial work remains record-free | Ledger prompt and every `ledger-*` skill |
| Shaping / orchestration / execution | Explicit responsibility states with sequential role handoffs when one session owns them | Ledger prompt, `ledger-brainstorming`, execution and orchestration skills |
| Assumption ratification | Execution-changing assumptions are record-backed, user-ratified, or blocking | Ledger prompt, task Assumptions, shaping/planning skills |
| Durable typed record graph | One task-local Ledger bundle, not `.10x/` | `docs/ledger.md`, `components/shared/src/ledger-system-prompt.ts` |
| Evidence over claims; red-team review | Procedure-and-limit evidence, worker reports as claims, independent `pi-review` | execution, review, verification skills |
| Retrospective compounding | Task Retrospective plus Distillation to docs, decisions, tests, runbooks, skills, or follow-up tasks | `ledger-finishing-a-development-branch`, `ledger-writing-skills` |
| Instruction autoresearch | Fresh disposable root-session baseline/treatment evaluation; no imported Python harness | `ledger-writing-skills`, task evaluation evidence |

The 13 incoming lifecycle directories and frontmatter names are `ledger-*`; `pi-exec`, `pi-review`, and `pi-ralph` retain their established names and boundaries. Runtime APIs remain authentic (`pi_exec`, `Agent`, `ledger_add`, `ledger_close`). Old generic lifecycle aliases and accidental `ledger-pi-*` aliases do not ship.

The optional visual companion is retained under `ledger-brainstorming`: authenticated local HTTP/WebSocket behavior and path/lifecycle safeguards are adapted from Superpowers; upstream branding and telemetry are removed; HTML may persist under task evidence while tokens, URLs, events, PIDs, and logs remain ephemeral. `spec-document-reviewer-prompt.md` and `ledger-writing-plans/plan-document-reviewer-prompt.md` supply the retained independent document-review gates.
