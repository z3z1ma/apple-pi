...
function parseWorkItems(section: Section, issues: WorkItemIssue[]): WorkItem[] {
	...
	for (const [index, line] of section.rawBody.split(/\r?\n/).entries()) {
		...
		if (!match) {
			issues.push({ code: "malformed_work_item", line: lineNumberValue, message: "Work Items may contain only canonical checkbox rows with a WI-### identifier, colon, and substantive description." });
			continue;
		}
		...
		if (state === "cancelled") {
			...
			if (!cancellation) {
				issues.push({ code: "malformed_work_item", line: lineNumberValue, message: `${id} is cancelled but omits a Cancelled: reason.` });
			} else {
				description = cancellation[1].trim();
				cancellationReason = cancellation[2].trim();
				if (!substantive(cancellationReason)) issues.push({ code: "non_substantive_cancellation", line: lineNumberValue, message: `${id} cancellation reason is not substantive.` });
			}
		}
		if (!substantive(description)) issues.push({ code: "non_substantive_work_item", line: lineNumberValue, message: `${id} description is not substantive.` });
		if (ids.has(id)) issues.push({ code: "duplicate_work_item", line: lineNumberValue, message: `Duplicate work item: ${id}` });
		ids.add(id);
...
export function parseTaskDocument(content: string): TaskDocument {
	...
	const sectionRanges = new Map(sections.map((section) => [section.heading, {
	...
	if (workItemsSection) {
		...
		if (acceptanceIndex === -1 || referencesIndex === -1 || workItemsIndex <= acceptanceIndex || workItemsIndex >= referencesIndex) {
			issues.push({ code: "misplaced_work_items_section", line: workItemsSection.line, message: "Work Items must appear between Acceptance Criteria and References." });
		}
	}
	...
	const workItemsEndLine = workItemsSectionIndex === -1 ? -1 : (sections[workItemsSectionIndex + 1]?.line ?? Number.MAX_SAFE_INTEGER) - 1;
	for (const [index, line] of content.split(/\r?\n/).entries()) {
		...
		if (looksLikeWorkItem(line) && (lineNumberValue <= workItemsStartLine || lineNumberValue > workItemsEndLine)) {
			issues.push({ code: "misplaced_work_item", line: lineNumberValue, message: "Work items must appear in the Work Items section." });
		}
	}
	return {
		headers,
