...
import { type Entry, type Observation, type Reflection, recallNotebookSources } from "../src/session-ledger/recall.js";
import {
...
function observationsEntry(id: string, observations: Observation[], coversUpToId = "src-1"): Entry {
	return {
		type: "custom",
		id,
		customType: NOTEBOOK_OBSERVATIONS_RECORDED,
...
function reflectionsEntry(id: string, reflections: Reflection[], coversUpToId = "src-1"): Entry {
	return {
		type: "custom",
		id,
		customType: NOTEBOOK_REFLECTIONS_RECORDED,
...
function dropsEntry(id: string, observationIds: string[], coversUpToId = "src-1"): Entry {
	return {
		type: "custom",
		id,
		customType: NOTEBOOK_OBSERVATIONS_DROPPED,
...
function retireEntry(id: string, reflectionIds: string[], coversUpToId = "src-1"): Entry {
	return {
		type: "custom",
		id,
		customType: NOTEBOOK_REFLECTIONS_RETIRED,
...
describe("session-ledger recall", () => {
	it("recalls an active observation with source entries", () => {
		...
		const result = recallNotebookSources(entries, OBS_1);
	...
	it("returns not_found for unknown ids", () => {
		...
		const result = recallNotebookSources(entries, "ffffffffffff");

		expect(result).toMatchObject({
			status: "not_found",
			notebookId: "ffffffffffff",
			collision: false,
			partial: false,
		});
	});

	it("reports collisions when an id matches multiple notebook records", () => {
		const entries = [
			sourceEntry("src-1"),
		...
		const result = recallNotebookSources(entries, SAME_ID);
