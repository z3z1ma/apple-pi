...
import { observationPoolMetrics } from "../src/maintenance/pool.js";
...
describe("Pair notebook observation pool metrics", () => {
	...
	it("uses folded active observations so tombstones reduce readiness", () => {
		...
		const entries = [
			...
			observationsRecordedEntry("notebook-obs", { observations: [dropped, active], coversUpToId: "raw-1" }),
			observationsDroppedEntry("notebook-drop", { observationIds: ["aaaaaaaaaaaa"], coversUpToId: "raw-1" }),
		];
