...
function notFound(notebookId: string): RecallResult {
	return {
		status: "not_found",
		notebookId,
		kind: undefined,
...
export function recallNotebookSources(entries: Entry[], notebookId: string): RecallResult {
	const {
	...
	const directObservationMatches = indexedObservations.filter(({ observation }) => observation.id === notebookId);
	const reflectionMatches = indexedReflections.filter(({ reflection }) => reflection.id === notebookId);

	if (directObservationMatches.length === 0 && reflectionMatches.length === 0) return notFound(notebookId);
