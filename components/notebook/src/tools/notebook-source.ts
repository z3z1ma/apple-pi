...
type RecallObservationToolStatus = "ok" | "partial" | "invalid_id" | "not_found" | "no_source" | "source_unavailable";
...
function sourceOriginAndQualifiers(entry: Entry): { origin: string; timestamp: string; qualifiers: string[] } {
	if (entry.type === "message" && entry.message && typeof entry.message === "object") {
		...
		return {
			origin: `Tool result: ${typeof toolName === "string" && toolName ? toolName : "unknown"}`,
			timestamp,
			qualifiers: [],
		};
	}
...
function friendlyNoSourceMessage(notebookId: string): string {
	return `Observation ${notebookId} has no source entries associated with it.`;
}
...
function directObservationMatches(result: Extract<RecallResult, { status: "found" }>): RecalledObservation[] {
	return result.observations.filter((match) => match.observation.id === result.notebookId);
}
...
function renderObservationOnlyTextFromResult(result: Extract<RecallResult, { status: "found" }>): string {
	...
	if (result.collision)
		sections.push(
			`Notebook id ${result.notebookId} matched multiple observations; returning all matching source results from the current branch.`,
		);
	for (const match of directObservationMatches(result)) {
		if (match.status === "dropped")
			sections.push(`Observation ${match.observation.id} is dropped from active notebook but remains recallable.`);
...
function renderNotebookText(result: Extract<RecallResult, { status: "found" }>): string {
	const sections: string[] = [];
	if (result.collision)
		sections.push(
			`Notebook id ${result.notebookId} matched multiple observations/reflections; returning all available evidence from the current branch.`,
		);
	...
	if (sections.length === 0)
		sections.push(`Notebook entry ${result.notebookId} was found, but no source evidence rendered.`);
	return sections.join("\n\n");
}
...
function resultDetails(
	...
	const detailWithoutStatus = {
		...
		observationId: result.notebookId,
		...
		sourceEntries,
		unavailableSupportingObservations: result.missingSupportingObservationIds.map((observationId) => ({
			observationId,
		})),
...
function renderFoundResult(result: Extract<RecallResult, { status: "found" }>): ReturnType<typeof textResult> {
	...
	const text = result.kind === "observation" ? renderObservationOnlyTextFromResult(result) : renderNotebookText(result);
...
function unavailableEvidenceMessage(_details: RecallObservationToolDetails): string {
	return "no source entries are available for this notebook id";
}
...
function notebookRows(details: RecallObservationToolDetails): string[] {
...
function noteRows(details: RecallObservationToolDetails, sources: RecallSourceEntryDetails[]): string[] {
	...
	if (details.status === "invalid_id") {
		notes.push(
			noteLine("invalid id", `notebook ids must be 12 lowercase hex characters; received ${details.notebookId}`),
		);
		return notes;
	}
	if (details.status === "not_found") {
		notes.push(
			noteLine(
				"not found",
				`no observation or reflection with id ${details.notebookId} was found on the current branch`,
			),
		);
		return notes;
	}
	if (details.collision) notes.push(noteLine("id collision", `multiple notebook entries share ${details.notebookId}`));
	...
	if (
		sources.length === 0 &&
		(details.reflections.length > 0 || details.observations.length > 0 || details.matches.length > 0)
	)
...
export function formatRecallResultForTui(
	...
	if (!details) {
		const text = result.content
			...
			.map((part) => part.text)
			.join("\n");
		return text || "notebook_source";
	}
	const sources = sourceEntriesFromDetails(details);
	const lines: string[] = [];
	const rows = notebookRows(details);
...
export const recallObservationTool = defineTool({
	...
	renderResult(result, options) {
		return new Text(
			formatRecallRenderedResultForTui(result as AgentToolResult<RecallObservationToolDetails>, options.expanded),
			0,
			0,
		);
	},
	async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
		...
		if (!NOTEBOOK_ID_PATTERN.test(notebookId)) {
			const message = `Notebook id must be 12 lowercase hex characters. Received: ${notebookId}`;
			return textResult(message, emptyDetails("invalid_id", notebookId, message));
		}
		const branchEntries = ctx.sessionManager.getBranch() as Entry[];
		const result = recallNotebookSources(branchEntries, notebookId);
		if (result.status === "not_found") {
			const message = `No observation or reflection with id ${notebookId} was found on the current branch.`;
			return textResult(message, emptyDetails("not_found", notebookId, message));
		}
