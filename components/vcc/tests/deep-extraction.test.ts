...
describe("outstanding context: deep error extraction", () => {
	...
	it("skips bash with exit code 0", () => {
		const blocks: NormalizedBlock[] = [{ kind: "bash", command: "echo ok", output: "ok", exitCode: 0 }];
		const r = buildSections({ blocks });
		expect(r.outstandingContext).toEqual([]);
	});

	it("skips bash with undefined exit code", () => {
		const blocks: NormalizedBlock[] = [{ kind: "bash", command: "echo ok", output: "ok", exitCode: undefined }];
		const r = buildSections({ blocks });
		expect(r.outstandingContext).toEqual([]);
	});
		...
		const blocks: NormalizedBlock[] = [
			{
				kind: "tool_result",
				name: "bash",
				text: "src/utils.ts(5,1): error TS2305: Module has no exported member 'foo'.",
				isError: true,
			},
		];
		const r = buildSections({ blocks });
		...
		const blocks: NormalizedBlock[] = [
			{
				...
				args: {
					file_path: "auth.ts",
					newText: "export function login(email: string, pwd: string): Promise<Session> {\n  // impl\n}",
				},
			},
			{ kind: "tool_result", name: "Edit", text: "ok", isError: false },
		];
		...
		const blocks: NormalizedBlock[] = [
			{
				kind: "tool_call",
				name: "Edit",
				args: { file_path: "auth.ts", newText: "export function login() {}\nexport function verifyToken() {}" },
			},
			{ kind: "tool_result", name: "Edit", text: "ok", isError: false },
		];
		const r = buildSections({ blocks });
