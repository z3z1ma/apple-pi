 ...
 *     say there is nothing more to fetch.
 *   - A separate continuation suffix exposes the real agent id to the model so
 *     it can resume this conversation. That id is for a new turn, not a cue that
 *     additional output from the settled turn remains to be fetched (#174).
 *
...
/**
 * Model-visible handle for continuing a settled in-process agent. Tool-result
 * `details` are renderer state and never reach the orchestrating model, so the
 * id must travel in content. Only advertise continuation when a live session
