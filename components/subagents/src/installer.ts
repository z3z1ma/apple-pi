...
export default function installSubagents(pi: ExtensionAPI): void {
	...
	const agentTool = defineTool({
		...
		promptGuidelines: [
			...
			"Use the Agent tool when you need collaboration: backgrounding, FleetView, steer/stop, resume, or a durable child session. Use pi_exec agents.run when you need a dynamic graph that composes specialists with core tools, MCP, or bound context. They share the same type catalog. Review planner/reviewer/verifier and Ralph executor/judge are program roles via custom systemPrompt, not catalog types.",
