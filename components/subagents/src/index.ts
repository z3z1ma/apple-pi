	...
	return {
		...
		callbacks: {
			onToolActivity: (activity: { type: "start" | "end"; toolName: string }) => {
				if (activity.type === "start") state.activeTools.set(`${activity.toolName}:${Date.now()}:${state.activeTools.size}`, activity.toolName);
				else {
...
function formatNotification(record: AgentRecord, maxLength: number): string {
...
function notificationDetails(record: AgentRecord, maxLength: number, activity?: AgentActivity): NotificationDetails {
	const output = record.result || record.error || "No output.";
	return {
		id: record.id,
...
function detailsFor(record: AgentRecord, activity?: AgentActivity, overrides: Partial<AgentDetails> = {}): AgentDetails {
...
export function renderRunningAgentStatus(frame: string, stats: string, activity: string, theme: Pick<Theme, "fg">): Container {
...
export default function installSubagents(pi: ExtensionAPI): void {
	...
	const queueNotification = (key: string, send: () => void) => {
		...
		pendingNotifications.set(key, setTimeout(() => {
			pendingNotifications.delete(key);
			send();
		}, 200));
	};
	...
	const groupJoin = new GroupJoinManager((records, partial) => {
		...
		queueNotification(key, () => {
			...
			if (rest.length) details.others = rest.map((record) => notificationDetails(record, 300, activityById.get(record.id)));
			pi.sendMessage<NotificationDetails>({
				customType: "subagent-notification",
	...
	manager = new AgentManager(
		...
		(record, info) => {
			if (!record.parentAgentId && !record.internalOwner) pi.events.emit("subagents:compacted", { id: record.id, ...info, compactionCount: record.compactionCount });
		},
	);
	...
	const managedService: ManagedSubagentService = {
		async runFresh(ctx, request) {
			...
			const { record } = await manager.spawnAndWait(pi, ctx, request.type, request.prompt, {
	...
	applySettings(settings, {
		...
		setDefaultMaxTurns,
		setGraceTurns,
		setDefaultJoinMode: (value) => { defaultJoinMode = value; },
		setStrictAgentFiles: (value) => { strictAgentFiles = value; },
		setDisableDefaultAgents: (value) => { setDefaultsDisabled(value); reloadAgents(); },
		setFleetView: (value) => { fleetEnabled = value; fleet.setEnabled(value); },
		setPersistAgentSessions,
		setWidgetMode: (value) => { widgetMode = value; widget.update(); },
		setMaxSubagentDepth,
		setFallbackSubagent,
	});
	...
	pi.registerMessageRenderer<NotificationDetails>("subagent-notification", (message, { expanded }, theme) => {
		...
		return new Text(all.map((details) => {
	...
	const availableDescription = () => getAvailableTypes().map((name) => {
	...
	const agentTool = defineTool({
		...
		async execute(toolCallId, params, signal, onUpdate, ctx) {
			...
			if (params.resume) {
				...
				if (!resumed) return textResult(`Agent ${existing.id} is already running or cannot be resumed.`, undefined, true);
				if (background) return textResult(`Agent resumed in background. Agent ID: ${existing.id}`, detailsFor(resumed, activity.state, { status: "background" }));
				return textResult(`${resumed.result || resumed.error || "No output."}${continuationSuffix(resumed)}`, detailsFor(resumed, activity.state), resumed.status === "error");
			}
			...
			const tracker = createActivityTracker(effectiveMaxTurns, () => {
				...
				if (id && onUpdate) {
					const record = manager.getRecord(id);
					if (record) onUpdate(textResult(tracker.state.responseText || "(running...)", detailsFor(record, tracker.state, {
						status: record.status,
			...
			const modelName = model && ctx.model && (model.provider !== ctx.model.provider || model.id !== ctx.model.id) ? model.id : undefined;
			...
			try {
				if (invocation.runInBackground) {
					...
					return textResult(`${dispatch.fellBackFrom !== undefined ? `Requested type fell back to ${type}.\n` : ""}Agent started in background. Agent ID: ${id}`, detailsFor(record, tracker.state, { status: "background" }));
				}
		...
		renderCall(args, theme) {
			...
			return new Text(`${theme.fg("toolTitle", theme.bold("Agent "))}${renderAgentName(type, theme, { bold: true })}${suffix}\n  ${theme.fg("dim", args.description || args.prompt || "...")}`, 0, 0);
		},
		renderResult(result, { expanded }, theme) {
			...
			if (details.status === "running" || details.status === "queued" || details.status === "background") {
				const stats = [details.turnCount ? formatTurns(details.turnCount, details.maxTurns) : "", details.toolUses ? `${details.toolUses} tools` : "", details.tokens].filter(Boolean).join(" · ");
				return renderRunningAgentStatus(SPINNER[details.spinnerFrame ?? 0], fgPreservingNestedStyles(theme, "dim", stats), details.activity || content, theme);
			}
			const failed = ["error", "aborted", "stopped"].includes(details.status);
			const icon = failed ? theme.fg("error", "✗") : theme.fg("success", "✓");
			const container = new Container();
			container.addChild(new Text(`${icon} ${renderAgentName(details.subagentType, theme, { bold: true })} ${theme.fg("dim", `${details.toolUses} tools · ${details.tokens} · ${formatMs(details.durationMs)}`)}`, 0, 0));
	...
	pi.registerTool(defineTool({
		...
		async execute(_toolCallId, params) {
			if (!manager.steer(params.agent_id, params.message)) return textResult(`Running agent not found: ${params.agent_id}`, undefined, true);
	...
	pi.registerTool(defineTool({
		...
		async execute(_toolCallId, params) {
			const record = manager.getRecord(params.agent_id);
			if (
				!record || record.parentAgentId || record.internalOwner ||
	...
	pi.registerCommand("agents", {
		...
		handler: async (_args, ctx) => {
			...
			const choices = [
				...records.map((record) => `${record.status === "running" ? "●" : "○"} ${record.description} · ${record.type} · ${record.id}`),
				"Agent types",
			];
