...
const A = await jiti.import(resolve(HERE, "index.ts"));
const M = await jiti.import(resolve(HERE, "lib/mode-utils.ts"));
...
async function loadAdvisorExtension() {
	...
	const res = await loadExtensions(["index.ts"], HERE, createEventBus(), runtime);
