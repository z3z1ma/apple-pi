...
export function createReviewAuthorityPolicy(projectRoot: string): ManagedAgentToolPolicy {
	return ({ toolName, args }) => {
		...
		for (const key of ["path", "root", "cwd"]) {
			...
			if (typeof value === "string" && containedProjectPath(projectRoot, value) === undefined) {
				return {
					block: true,
					reason: `${toolName}.${key} escapes the trusted review project or targets .git`,
					terminate: true,
				};
			}
		}
