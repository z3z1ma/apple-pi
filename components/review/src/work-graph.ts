...
export function compileReviewWorkGraph(
	...
	if (output.groups.length > maxGroups) throw new ReviewGraphError(`Planner returned ${output.groups.length} groups; maximum is ${maxGroups}`, "too_many_groups");
	...
	const groups = output.groups.map((group, index) => {
		...
		if (group.itemIds.length === 0) throw new ReviewGraphError(`Review group ${group.id} has no focus items`, "empty_group");
		for (const id of group.itemIds) {
			if (!expected.has(id)) throw new ReviewGraphError(`Review group ${group.id} invented item ID ${id}`, "unknown_item");
			const prior = assigned.get(id);
			if (prior) throw new ReviewGraphError(`Review item ${id} appears in both ${prior} and ${group.id}`, "duplicate_item");
			assigned.set(id, group.id);
		}
		for (const path of group.contextPaths) {
			if (!safeRelativePath(path)) throw new ReviewGraphError(`Review group ${group.id} has unsafe context path: ${path}`, "unsafe_context_path");
		}
		return {
			...group,
	...
	if (missing.length > 0) {
		throw new ReviewGraphError(`Planner omitted review items: ${missing.map((item) => item.path).join(", ")}`, "missing_items");
	}
