...
function harness(result: AdvisorConsultationResult, minTurnsBetween = 0) {
	...
	const outcomes: any[] = [];
	let ready = 0;
	const controller = new SentinelEscalationController({
		pi,
		getContext: () => ctx,
...
describe("Sentinel escalation machinery", () => {
	it("keeps escalation private and excludes delegation or mutation capabilities", () => {
		expect(SENTINEL_SESSION_TOOLS).toContain("escalate");
		for (const forbidden of ["Agent", "pi_exec", "bash", "edit", "write", "mcp"]) {
			expect(SENTINEL_SESSION_TOOLS).not.toContain(forbidden);
		}
	});
	...
	it("delivers a current confirmed finding and records structural usage", async () => {
		...
		expect(h.controller.submit("sentinel", request, 1)).toBe("accepted");
		await vi.waitFor(() => expect(h.ready).toBe(1));
		const delivered: any[] = [];
		await h.controller.flushDelivery((note) => delivered.push(note));
	...
	it("suppresses a refutation and collapses an equivalent concurrent escalation", async () => {
		...
		expect(h.controller.submit("sentinel", request, 1)).toBe("accepted");
		expect(h.controller.submit("sentinel", request, 1)).toBe("suppressed");
		settle({
			status: "completed",
		...
		await h.controller.flushDelivery(() => {
			throw new Error("refutations must not be delivered");
		});
	...
	it("drops a late finding when implicated working state changed", async () => {
		...
		h.controller.submit("sentinel", request, 1);
		await vi.waitFor(() => expect(h.ready).toBe(1));
		h.changeState();
		const delivered: any[] = [];
		await h.controller.flushDelivery((note) => delivered.push(note));
	...
	it("labels provider failure as unadjudicated instead of confirmation or refutation", async () => {
		...
		const delivered: any[] = [];
		await h.controller.flushDelivery((note) => delivered.push(note));
		expect(delivered[0]).toMatchObject({ source: "sentinel", adjudication: "unadjudicated" });
	...
	it("throttles starts by turns without imposing an absolute consultation maximum", async () => {
		...
		h.controller.submit("sentinel", changedEvidence, 1);
