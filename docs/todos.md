...
Project storage is an opt-in trusted-project mode at `<cwd>/.pi/todos/shared.json`. It is shared across sessions and processes, but storage-mode changes take effect only at the next session start and do not migrate lists. Each mutation re-reads under an exclusive same-directory lock, writes a uniquely named temporary file, and atomically renames it. Lock cleanup is token-checked; stale locks are removed only when their owner PID is positively dead.
