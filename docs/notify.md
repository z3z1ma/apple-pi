...
- Terminal provider errors, failed or cancelled context compaction, and cancelled tasks are reported explicitly — including `cyber_policy` blocks — instead of being shown as successful completion. A compaction failure is provisional: a later successful assistant response clears it, so an automatic recovery still reports completion.
