...
The `agent` tool brings a named teammate into a foreground or background Pi session. `get_subagent_result` waits for or checks background work, `steer_subagent` gives a running teammate more guidance after their current tool, and `stop_subagent` ends queued or running work. `/agents` opens a focused roster of public top-level agents and a browser for discovered built-in and Markdown agent types. In TUI mode, running and queued public agents appear in the shared passive above-editor active-work widget, and the input card shows a terse non-zero `agents:N` count. Terminal outcomes stay in transcript notifications instead of lingering in the passive surface.

The `/agents` roster orders and labels live and recently settled records, preserves selection by agent ID after inspecting a conversation, and follows configured selection keybindings. Press `Enter` to inspect the selected conversation, `t` to switch between the roster and discovered agent types, and `Esc` or `q` to close. In conversation detail, use the configured scroll keys, `Enter` to compose a steering message for a running or queued agent, `x` twice to confirm stopping active work, and `Esc` or `q` to return. Settled conversations remain readable without steer or stop actions. Nested and internally owned agents are excluded from this flow.
