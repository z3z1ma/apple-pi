...
async function runAgent(
	...
	return await new Promise((resolve) => {
		...
		child.on("close", (code) => {
			...
			const output = bounded(stdout || error || "(agent returned no text)", MAX_NESTED_RESULT_CHARS, "pi_exec agent output");
			resolve({
				index,
				task: request.task,
				output: output.value,
...
function serializeHostCallOutcome(ok: boolean, value: unknown): string {
	try {
		return JSON.stringify(ok
			? { ok: true, value }
			: { ok: false, error: value instanceof Error ? value.message : String(value) },
...
function traceFetchUrl(value: unknown): string {
...
function fetchOperationArgs(args: Record<string, unknown>): Record<string, unknown> {
...
async function executeFetch(args: Record<string, unknown>, signal: AbortSignal): Promise<Record<string, unknown>> {
	if (typeof args.url !== "string" || args.url.trim() === "") {
		throw new TypeError("fetch requires an absolute URL");
	}
	if (
		args.headers !== undefined &&
		(!Array.isArray(args.headers) || args.headers.some((header) =>
			!Array.isArray(header) || header.length !== 2 || header.some((value) => typeof value !== "string")
		))
	) {
...
function resultText(result: any): string {
	...
	return result.content.map((part: any) =>
		part?.type === "text" ? String(part.text ?? "") : `[${part?.mimeType ?? "image"}]`
	).join("\n");
}
...
export default function runtime(pi: ExtensionAPI): void {
	...
	pi.registerTool({
		...
		promptSnippet: "pi_exec: compose core Pi tools, fetch, and subagents with branching, fan-out, pipelines, and reduction",
		promptGuidelines: [
		...
		parameters: Type.Object({
			...
			inputs: Type.Optional(Type.Record(Type.String(), Type.String({ maxLength: 200_000 }), {
		...
		async execute(toolCallId, params, signal, onUpdate, ctx) {
			...
			if (ctx.hasUI && ctx.mode === "tui") {
				try {
					...
					ctx.ui.notify(`pi_exec activity widget unavailable: ${error instanceof Error ? error.message : String(error)}`, "warning");
				}
			}
			...
			const hostCall: ProgramHostCall = async (ref, rawArgs, runtimeSignal) => {
				...
				try {
					...
					if (ref === "fetch") {
						...
						const name = typeof rawArgs.name === "string" ? rawArgs.name : "";
						const args = rawArgs.args && typeof rawArgs.args === "object" && !Array.isArray(rawArgs.args)
						...
						value = result.error
							? { status: "failed", error: result.error, text: result.output, toolCalls: result.operations.length, ...(result.usage ? { usage: result.usage } : {}) }
							: { status: "completed", text: result.output, toolCalls: result.operations.length, ...(result.usage ? { usage: result.usage } : {}) };
						if (result.error) {
					...
					operation.result = ref === "fetch" && value && typeof value === "object"
						? {
			...
			try {
				...
				if (widgetMounted) {
					try {
						...
						ctx.ui.notify(`pi_exec activity widget cleanup failed: ${error instanceof Error ? error.message : String(error)}`, "warning");
					}
				}
			}
		},
