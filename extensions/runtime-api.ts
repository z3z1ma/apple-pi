...
export const STD_GUEST_LIBRARY_SIGNATURES: string[] = [
	"std: Readonly<Std>. Frozen, non-replaceable, and limited to capabilities that materially improve on pi.*, pi.bash, fetch, agent(), agent.run(), parallel, or pipeline. It adds no authority and exposes no generic shell, filesystem, HTTP, graph, agent, semantic, or mutation wrappers.",
	...
	"type SchemaPrimitive = 'string'|'int'|'number'|'boolean'; type SchemaConstraint = { string: { minLength?: number, maxLength?: number } } | { int: { minimum?: number, maximum?: number } } | { number: { minimum?: number, maximum?: number } }; type SchemaArrayConstraint = { array: { minItems?: number, maxItems?: number }, items: [SchemaShape] }; type SchemaShape = SchemaPrimitive | `$" +
		"{SchemaPrimitive}?` | [SchemaShape] | [string, string, ...string[]] | { [key: string]: SchemaShape } | SchemaConstraint | SchemaArrayConstraint. std.schema(shape: SchemaShape) → JsonSchema. Compiles terse strict shapes; fields are required and additionalProperties is false by default.",
...
export const GUEST_HELPER_SIGNATURES: string[] = [
	...
	"state: Record<string, JSONValue>  // mutable JSON data; when changed, the result returns a state id that resumes this snapshot in a later live-session call",
	...
	"type AgentRequest = string | { task: string, type?: string, name?: string, profile?: InferenceProfile, tools?: AgentTool[], pair?: boolean, systemPrompt?: string, context?: JSONValue, outputSchema?: object }",
	...
	"agent.run(request: AgentRequest) → Promise<AgentRunResult>",
	...
	"Workers load Codex fast mode, the proactive overflow guard, the home search guard, ledger, and search_session extensions. They do not load pi_exec or the subagent manager, and they cannot call MCP. Call those in the program, then bind the compact result as context.",
...
export const PI_EXEC_PROMPT_GUIDELINES = [
	...
	"For expensive data worth reusing across calls, mutate the state global; the result returns a state id to pass as the next call's state parameter.",
	...savedProgramsSystemPromptContribution.guidelines,
	`The code parameter is a JavaScript async-function body. Core calls are pi.read/grep/find/ls/bash/edit/write and each takes one object matching that parent tool, never a positional string: pi.read({ path }), pi.grep({ pattern }), pi.bash({ command }). fetch, skills.list, skills.body({ name }), agent(), agent.run(), parallel/pipeline, and every captured extension tool including the MCP gateway are listed on the code parameter before you write the program. agent options may include task, type, name, profile, tools, pair, systemPrompt, context, and outputSchema. The live <${TEAM_SYSTEM_PROMPT_TAG}> block lists every callable teammate with its name, configured inference profile, and own description. The separate <${INFERENCE_PROFILES_SYSTEM_PROMPT_TAG}> block lists the inference profiles and their descriptions. type selects a teammate; profile selects an inference profile; The builder enables the pair programmer by default, and pair: false explicitly opts out; systemPrompt appends dynamic specialization without replacing the teammate definition or granting capabilities. Explicit tools override capabilities. Omit type for a generic read-only worker. The root agent tool provides the equivalent subagent_type, profile, and system_prompt combination, but interactive subagent tools are not captured by extensions.*. Review and Ralph may remain untyped program workers with explicit system prompts. Call MCP and extension tools in the program, then bind compact results as agent context. Use agent.run for fan-out so one failed worker does not abort the program. Load the pi-exec skill for gather-then-bind examples. The agent tool is for root-session collaboration (background, steer, resume); agent.run is for in-program graphs.`,
