...
export const STD_GUEST_API_FUNCTIONS = {
	...
	context: ["required", "clippable", "droppable", "fit", "pack", "partition"],
	evidence: ["item", "bundle", "pack", "require"],
...
export const STD_GUEST_LIBRARY_SIGNATURES: string[] = [
	...
	"type GitNameStatus = { status: string, code: string, path: string, from?: string }; type GitStatusEntry = { index: string, worktree: string, path: string, untracked: boolean, from?: string }; type GitChange = { compare: string, paths?: string[], status: { entries: GitStatusEntry[], dirty: boolean, untrackedFiles: string[] }, stat: string, patch: string, changedFiles: string[], untrackedFiles: string[], renames: GitNameStatus[], additions: number, deletions: number, nameStatus: GitNameStatus[] }.",
	"std.git.change(options?: GitOptions) → Promise<GitChange>. Collects and normalizes status, stat, patch, name-status, untracked files, renames, and line totals in one bounded operation; read-only.",
	...
	"std.context.fit<T>(value: T, { maxSerializedChars?: number }?) → { value: Unmarked<T>, truncated: string[], dropped: string[], serializedChars: number }. Drops droppable fields, unwraps all retained marks, clips marked strings by priority, and throws if required data cannot fit.",
	"std.context.pack<T>(items: T[], { maxSerializedChars?: number, priority?: Key<T>, id?: Key<T> }?) → { items: T[], omitted: T[], omittedIds: unknown[], serializedChars: number }; std.context.partition<T>(items: T[], { maxSerializedChars?: number }?) → T[][]. pack prioritizes items within one budget; partition creates budget-fitting batches and throws when one item cannot fit.",
	"type EvidenceItem = { id: string, kind: string, source: string, locator: object, content: string, truncated: boolean }; type EvidenceBundle = { items: EvidenceItem[] }; std.evidence.item(input?: { id?: string, kind?: string, source?: string, locator?: object, content?: unknown, truncated?: boolean }) → EvidenceItem; std.evidence.bundle(items: Array<EvidenceItem | Partial<EvidenceItem>>) → EvidenceBundle; std.evidence.pack(bundle: EvidenceBundle, { maxSerializedChars?: number, priority?: Key<EvidenceItem> }?) → { bundle: EvidenceBundle, omittedIds: string[], serializedChars: number }; std.evidence.require(bundle: EvidenceBundle, ids: string[]) → EvidenceItem[] (throws when an ID is absent).",
	"type Coverage<T> = { covered: T[], missing: T[], unexpected: T[], duplicates: T[], complete: boolean }; std.coverage.compare<T>(expected: T[], actual: T[], { id?: Key<T>, key?: Key<T> }?) → Coverage<T>; std.coverage.requireComplete<T>(expected: T[], actual: T[], options?: { id?: Key<T>, key?: Key<T> }) → Coverage<T> (throws unless complete). id and key are aliases.",
	"std.reconcile.byId<T, U>(base: T[], overlays: U[], { id?: Key<T>, key?: Key<T>, overlay?: boolean }?) → { values: Array<T | U>, unknownIds: unknown[], missingIds: unknown[], duplicateIds: unknown[] }; std.reconcile.oneToOne<T, U>(expected: T[], actual: U[], { id?: Key<T>, key?: Key<T> }?) → { pairs: Array<T | U>, covered: T[], missing: T[], unexpected: U[], duplicates: U[], complete: boolean }. id and key are aliases; byId defaults to the 'id' field, while oneToOne defaults to primitive/object identity.",
	"std.agents.planFanoutReduce(input: JSONValue, { plan: { prompt: string, profile: InferenceProfile, outputSchema?: JsonSchema, tools?: AgentTool[], systemPrompt?: string, context?: object }, fanout: { profile: InferenceProfile, outputSchema?: JsonSchema, tools?: AgentTool[], systemPrompt?: string, context?: object, concurrency?: number }, reduce: { prompt: string, profile: InferenceProfile, outputSchema?: JsonSchema, tools?: AgentTool[], systemPrompt?: string, context?: object } }) → Promise<{ plan: { fanout: Array<{ prompt: string, name?: string, context?: object }> } & JSONValue, results: AgentRunResult[], value: string | JSONValue }>. All profiles are required; omitted plan.outputSchema defaults to the shown fanout object schema; reducer context contains input, plan, and failure-preserving results. The topology adds no authority but can perform caller-authorized effects through any stage explicitly granted bash, edit, or write.",
	"type ReadOnlyAgentTool = 'read'|'grep'|'find'|'ls'; type DevAnalysisOptions = GitOptions & { instruction: string, schema: JsonSchema, profile?: InferenceProfile, tools?: ReadOnlyAgentTool[] }; std.dev.analyzeChange(options: DevAnalysisOptions) → Promise<JSONValue satisfying options.schema>. Collects change-neighborhood evidence, then runs one typed read-only worker; instruction and schema are required.",
	"std.dev.analyzeFailure(options: DevAnalysisOptions & ({ failure: JSONValue, logs?: JSONValue } | { logs: JSONValue, failure?: JSONValue })) → Promise<JSONValue satisfying options.schema>. Binds failure/log data plus change-neighborhood evidence to one typed read-only worker; instruction, schema, and failure or logs are required.",
