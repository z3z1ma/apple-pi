...
export const GUEST_HELPER_SIGNATURES: string[] = [
	...
	"tools.call({ name: string, args?: object }) → Promise<ToolResult>",
	"skills.list() → Promise<Array<{ name: string, description: string }>>  // model-invocable session skills; human-only skills stay hidden",
	"skills.body({ name: string }) → Promise<string>  // model-invocable SKILL.md body, frontmatter stripped; throws if missing or human-only",
