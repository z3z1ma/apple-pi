...
There is no single success story for the [advisor](docs/advisor.md). The value is watching a transcript and seeing a nit, concern, or blocker appear while the model is implementing the code. Every time I wonder what would have happened without it. Another review cycle? A bug nobody noticed? Instead the trajectory changes while the work is still happening. It also lets a weaker model execute with a stronger model watching a compressed view of the run. The advisor stays read-only because there should still be one agent responsible for changing the code.
