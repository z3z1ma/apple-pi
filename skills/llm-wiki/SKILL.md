...
Use ordinary tools directly for small operations. Use existing Research, Explore, MCP, or `pi_exec` capabilities only when source acquisition or bounded fan-out materially helps. Acquisition and synthesis are separate boundaries: obtain and inspect a source before changing wiki knowledge, and keep fetch, extraction, privacy, or format failures visible. Do not create wiki-specific runtime machinery.
