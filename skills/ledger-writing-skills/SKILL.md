---
name: ledger-writing-skills
description: "Use when creating, editing, or verifying a Ledger-backed Agent Skill, especially after recurring toil or an observed instruction failure."
---
...
**Pi discovery:** package skills live under this package's `skills/` directory and are registered by `package.json`; personal skills live under `~/.pi/agent/skills/` or `~/.agents/skills/`; trusted project skills live under `.pi/skills/` or `.agents/skills/`. Pi lists each skill's exact location in the available-skills catalog.

You write test cases (pressure scenarios in fresh disposable root Pi sessions), watch them fail (baseline behavior), write the skill (documentation), watch tests pass (agents comply), and refactor (close loopholes).
...
**REQUIRED BACKGROUND:** You MUST understand ledger-test-driven-development before using this skill. That skill defines the fundamental RED-GREEN-REFACTOR cycle. This skill adapts TDD to documentation.
...
|-------------|----------------|
| **Test case** | Pressure scenario in a fresh root Pi session |
...
the operator: "How did we handle authentication errors in React Router before?"
...
- ✅ Good: `**REQUIRED SUB-SKILL:** Use ledger-test-driven-development`
- ✅ Good: `**REQUIRED BACKGROUND:** You MUST understand ledger-systematic-debugging`
- ❌ Bad: `See skills/testing/ledger-test-driven-development` (unclear if required)
- ❌ Bad: `@skills/testing/ledger-test-driven-development/SKILL.md` (force-loads, burns context)
...
**Visualizing for the operator:** Use `render-graphs.js` in this directory to render a skill's flowcharts to SVG:
```bash
...
**REQUIRED BACKGROUND:** The ledger-test-driven-development skill explains why this matters. Same principles apply to documentation.
...
**Examples:** TDD, ledger-verification-before-completion, designing-before-coding
...
Run the pressure scenario in a fresh disposable root Pi session WITHOUT the skill. Document exact behavior:
...
1. **One fresh-context sample per call** — a raw API call, or a single-shot disposable root Pi session. System prompt = the realistic context the guidance will live in (the full skill or prompt template, not the guidance in isolation); user message = a task that tempts the failure.
