...
**Announce at start:** "I'm using the ledger-writing-plans skill to create the implementation plan."

**Context:** If working in an isolated worktree, it should have been created via the `ledger-using-git-worktrees` skill at execution time.

## Ledger State: Shaping To Orchestration

A plan is the cold-start bridge from ratified behavior to bounded execution. It must preserve why each Work Item exists, its dependencies and integration points, the exact source surfaces it owns, the failure that proves RED, and the observation that can prove done. Plans may choose reversible implementation mechanics; they may not turn examples, current accidents, or technical convenience into product authority. If a Work Item still requires an execution-changing semantic choice, return it to `ledger-brainstorming` instead of hiding the choice in implementation steps.

## Ledger Authority

Find the owning task through `.ledger/INDEX.md`, live task roots, and `.ledger/history/INDEX.md`. Extend the existing owner when its Scope and Acceptance Criteria cover the outcome. For new non-trivial work, use `ledger_add`, then complete ledger-brainstorming before planning.
...
If the spec covers multiple independent subsystems, it should have been broken into sub-project specs during ledger-brainstorming. If it wasn't, suggest breaking this into separate plans — one per subsystem. Each plan should produce working, testable software on its own.
...
> **For executors:** REQUIRED SUB-SKILL: Use ledger-subagent-driven-development (recommended) or ledger-executing-plans to implement this plan Work-Item by Work-Item. `task.md` owns the canonical `WI-###` checklist.
...
- **REQUIRED SUB-SKILL:** Use ledger-subagent-driven-development
- Fresh typed implementer per Work Item + independent review

**If Inline Execution chosen:**
- **REQUIRED SUB-SKILL:** Use ledger-executing-plans
