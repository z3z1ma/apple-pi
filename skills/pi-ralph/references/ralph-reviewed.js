...
async function reviewChange(files, background, priorFindings, priorCoverageGaps, priorRisks = []) {
	...
	const planningContext = contextWithPatch(
		{
			...
			background,
			compare,
			changeStatus: statusSummary(change.status),
	...
	const verifierCandidates = std.context.pack(
		...
		{ maxSerializedChars: 12000, id: "candidateId", priority: "priority" },
	);
	...
	const packedPriorCoverageGaps = std.context.pack(
		priorCoverageGapRecords.map((gap) => ({
			id: gap.id,
			message: compactText(gap.message, 500),
		})),
		{
	...
	const packedPriorRisks = std.context.pack(
		priorRiskRecords.map((risk) => ({
			id: risk.id,
			kind: risk.kind,
			message: compactText(risk.message, 700),
		})),
		{
			maxSerializedChars: 8000,
			id: "id",
	...
	const meta = await agent({
		...
		outputSchema: {
			...
			required: [
				...
				"residualRisks",
				"coverageGaps",
				"priorCoverageGapIdsAssessed",
				"priorRiskIdsAssessed",
			],
			properties: {
				decisions: {
					type: "array",
				...
				priorCoverageGapIdsAssessed: { type: "array", items: { type: "string" } },
				priorRiskIdsAssessed: { type: "array", items: { type: "string" } },
			},
		},
	});
...
function reviewFeedback(findings, coverageGaps, priorRisks = []) {
	const packedFindings = std.context.pack(
		findings.map((finding, index) => ({
			...
			trigger: compactText(finding.trigger, 500),
			evidence: compactText(finding.evidence, 700),
			impact: compactText(finding.impact, 400),
			recommendation: compactText(finding.recommendation, 300),
		})),
		{ maxSerializedChars: 12000, id: "id", priority: "priority" },
	);
	const gapRecords = coverageGapRecords(coverageGaps);
	const packedGaps = std.context.pack(
		gapRecords.map((gap) => ({ id: gap.id, message: compactText(gap.message, 500) })),
		{ maxSerializedChars: 8000, id: "id" },
	);
