...
You may read and write the repository through the tools granted by the Ralph program. Leave implementation changes uncommitted. Do not commit, push, reset, hide changes, or claim that a review was performed. Review remains a caller-owned `/skill:pi-review` step for the default loop.
