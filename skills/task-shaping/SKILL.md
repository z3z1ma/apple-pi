...
- **Architectural** — new projects, new subsystems, changes that
  ...
  design, written spec, then the `implementation-planning` skill.
...
7. **Independent spec review** — load `review-commissioning`, use its executable review gate in `specification` mode, translate [spec-document-reviewer-prompt.md](spec-document-reviewer-prompt.md) into the gate inputs, and resolve material findings
8. **User reviews written spec** — ask user to review the spec file before proceeding
9. **Transition to implementation planning** — load `implementation-planning` and create the source-backed plan

## Process Flow

```dot
digraph task-shaping {
    ...
    "Load implementation-planning" [shape=doublecircle];
    ...
    "User reviews spec?" -> "Load implementation-planning" [label="approved"];
}
```
...
- Load `implementation-planning` from its catalog location and create a detailed implementation plan.
