...
Once the prototype has answered its question, capture the answer, then capture the prototype the way the [SKILL](../SKILL.md) describes. The logic-specific mapping: when production adoption is authorized, the validated reducer / machine / function set lifts into the real module (the decision, absorbed); the HTML shell remains in the active ledger task as the prototype's primary source, and being one self-contained file, it stays trivially re-runnable there.
