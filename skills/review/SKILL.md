...
`review` owns review composition and finding semantics. `ralph` owns the caller-bounded fresh-context implementation loop, and its default loops remain separate from review. The `ralph` skill's `ralph-ledger-review.js` is an opt-in advanced example that composes a Ledger Ralph increment with this review spine; it is not a default workflow or permission to make review implicit.
