...
import {
	...
	receiptPath,
	readReceiptEvents,
} from "../components/ralph/src/receipts.js";
...
import {
	...
	mutateTaskWorkItems,
} from "../components/ralph/src/task.js";
	...
	it("rejects malformed or incomplete structured role output", () => {
		...
		expect(() =>
			parseExecutorOutput({
				...
				journal: [],
				blockers: [],
				retrospective: "learned",
			}),
		).toThrowError(/distillation/);
		expect(() =>
			parseExecutorOutput({
				status: "done",
				...
				journal: [],
				blockers: [],
				retrospective: "A durable lesson was recorded.",
				distillation: [],
				workItemCompletions: [{ id: "WI-001", evidence: "Substantive completion evidence.", cancel: true }],
			}),
		).toThrowError(/unsupported fields/);
		expect(() => parseJudgeOutput({ decision: "iterate", reason: "more", acceptanceCriteria: [] })).toThrowError(
			/nextObjective/,
		);
		expect(() =>
			parseJudgeOutput({
				decision: "close",
				reason: "The evidence and review agree.",
				acceptanceCriteria: [],
				workItemJudgments: [
					{ id: "WI-001", decision: "confirmed", reason: "Substantive judgment reason.", applied: true },
				],
			}),
		).toThrowError(/unsupported fields/);
	});
...
describe("task recording and receipts", () => {
	it("records independent review and judgment in task.md before deterministic closure", async () => {
		...
		await appendJudgment(graph.task.absolutePath, graph.task.digest, "run-12345678", 1, {
			...
			acceptanceCriteria: [{ id: "AC-001", status: "satisfied", evidence: "Task observation." }],
			workItemJudgments: [],
		});
		graph = compileWorkGraph(root, TASK);
	...
	it("refuses to add open work to a done task", async () => {
		...
		await expect(
			mutateTaskWorkItems(path, closed.digest, {
				kind: "add",
				id: "WI-001",
	...
	it("rejects post-judgment work-item drift before leased completion", async () => {
		...
		const path = join(root, TASK);
		let graph = compileWorkGraph(root, TASK);
		await mutateTaskWorkItems(path, graph.task.digest, {
			kind: "add",
			id: "WI-001",
			description: "Implement the authority-preserving completion boundary.",
		});
		graph = compileWorkGraph(root, TASK);
		const judged = await appendJudgment(path, graph.task.digest, "controller-owner", 1, {
			...
			workItemJudgments: [
				{ id: "WI-001", decision: "confirmed", reason: "The reviewed change supports this completion." },
			],
		});
		writeFileSync(
			path,
			readFileSync(path, "utf8").replace(
				"Implement the authority-preserving completion boundary.",
				"A concurrent edit changed the judged work-item description.",
			),
		);
		...
		await expect(completeTaskWorkItemsUnderLease(path, judged.digest, "controller-owner", ["WI-001"])).rejects.toThrow(
			/changed concurrently/,
		);
		release();
		expect(readFileSync(path, "utf8")).toContain(
			"- [ ] WI-001: A concurrent edit changed the judged work-item description.",
		);
	});
	...
	it("mutates work items under the task-bundle lease and preserves failed task bytes", async () => {
		...
		let mutation = await mutateTaskWorkItems(path, digest, {
			kind: "add",
			id: "WI-001",
			description: "Implement the canonical work-item mutation boundary.",
		});
		digest = mutation.digest;
		mutation = await mutateTaskWorkItems(path, digest, {
			kind: "add",
			id: "WI-002",
			description: "Preserve atomic failures under the task-bundle lease.",
		});
		digest = mutation.digest;
		...
		await expect(
			mutateTaskWorkItems(path, digest, {
				kind: "add",
				id: "WI-003",
		...
		await expect(
			mutateTaskWorkItems(path, mutation.digest, {
				kind: "add",
				id: "WI-003",
				description: "Foreign leases must retain exclusive task authority.",
			}),
		).rejects.toThrow(/already owns/);
		release();

		mutation = await mutateTaskWorkItems(path, mutation.digest, {
			kind: "add",
			id: "WI-003",
			description: "Controller confirmation needs an owned task-bundle lease.",
		});
		await expect(completeTaskWorkItemsUnderLease(path, mutation.digest, "missing-owner", ["WI-003"])).rejects.toThrow(
			/does not own/,
		);
	...
	it("leases the implementation workspace and authoritative task independently", () => {
		...
		expect(() => acquireRalphRunLeases(workspaceB, root, TASK, "bbbbbbbb-run")).toThrowError(
	...
	it("persists schema-v2 recoverable state and refuses legacy resume", async () => {
		...
		await expect(appendReceipt(run, { outcome: "malformed", workItems: null as never })).rejects.toThrow(
			/workItems is invalid/,
		);
		await expect(
			appendReceipt(run, {
				stage: "executor",
				outcome: "done",
				workItems: { proposals: [{ id: "WI-001", evidence: "The completion evidence is substantive." }] },
			}),
		).rejects.toThrow(/invalid proposal event/);
		await appendReceipt(run, { outcome: "started" });
		run.state = "executing";
		run.iteration = 1;
		await appendReceipt(run, { stage: "executor", outcome: "stage_started" });
		run.state = "reviewing";
		await appendReceipt(run, { stage: "reviewer", outcome: "stage_started" });
		run.state = "judging";
		await appendReceipt(run, { stage: "judge", outcome: "stage_started" });
		run.state = "iterating";
		await appendReceipt(run, { outcome: "iteration_complete" });
		run.state = "executing";
		run.iteration = 2;
		const proposals = [{ id: "WI-001", evidence: "Implemented and validated the bounded mutation." }];
		await expect(appendReceipt(run, { stage: "reviewer", outcome: "done", workItems: { proposals } })).rejects.toThrow(
			/invalid proposal event/,
		);
		await expect(
			appendReceipt(run, { stage: "executor", outcome: "done", workItems: { proposals, unexpected: true } as never }),
		).rejects.toThrow(/invalid envelope/);
		await expect(
			appendReceipt(run, {
				stage: "executor",
				outcome: "done",
				workItems: { proposals: [{ ...proposals[0], unexpected: true }] as never },
			}),
		).rejects.toThrow(/invalid proposal event/);
		await expect(
			appendReceipt(run, {
				stage: "executor",
				outcome: "done",
				workItems: { proposals: [{ id: "WI-001", evidence: "todo" }] },
			}),
		).rejects.toThrow(/invalid proposal event/);
		await appendReceipt(run, { stage: "executor", outcome: "done", workItems: { proposals } });
		await expect(appendReceipt(run, { stage: "executor", outcome: "done", workItems: { proposals } })).rejects.toThrow(
			/duplicate work-item proposals/,
		);
		run.state = "reviewing";
		await appendReceipt(run, { stage: "reviewer", outcome: "stage_started" });
		run.state = "judging";
		const judgments = [
			{ id: "WI-001", decision: "confirmed" as const, reason: "The supplied evidence is sufficient." },
		];
		await expect(
			appendReceipt(run, { stage: "judge", outcome: "iterate", workItems: { judgments: [] } }),
		).rejects.toThrow(/does not assess work-item proposals exactly/);
		await expect(
			appendReceipt(run, {
				stage: "judge",
				outcome: "iterate",
				workItems: { judgments: [{ ...judgments[0], unexpected: true }] as never },
			}),
		).rejects.toThrow(/invalid judgment event/);
		await expect(
			appendReceipt(run, {
				stage: "judge",
				outcome: "iterate",
				workItems: { judgments: [{ id: "WI-001", decision: "confirmed", reason: "pending" }] },
			}),
		).rejects.toThrow(/invalid judgment event/);
		await appendReceipt(run, { stage: "judge", outcome: "iterate", workItems: { judgments } });
		await expect(appendReceipt(run, { stage: "judge", outcome: "iterate", workItems: { judgments } })).rejects.toThrow(
			/repeats work-item judgments/,
		);
		...
		await expect(
			appendReceipt(run, {
				...
				workItems: { confirmedIds: ["WI-001"], rejectedIds: ["WI-001"], taskDigest },
			}),
		).rejects.toThrow(/invalid applied work-item state/);
		await expect(
			appendReceipt(run, {
				stage: "judge",
				outcome: "work_items_applied",
				workItems: { confirmedIds: ["WI-999"], rejectedIds: [], taskDigest },
			}),
		).rejects.toThrow(/invalid applied work-item state/);
		await appendReceipt(run, {
			stage: "judge",
			outcome: "work_items_applied",
			workItems: { confirmedIds: ["WI-001"], rejectedIds: [], taskDigest },
		});
		await expect(
			appendReceipt(run, {
				stage: "judge",
		...
		).rejects.toThrow(/repeats applied work-item state/);
		run.state = "iterating";
		await appendReceipt(run, { outcome: "iteration_complete" });
		...
		await appendReceipt(run, {
			outcome: "gate",
			gate: { kind: "error", reason: "Ralph executor did not submit exactly one typed result" },
		});
