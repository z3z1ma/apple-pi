...
`,
    ...
    expect(calls[0].args.headers).toEqual(expect.arrayContaining([
...
`,
      async (_ref, _args, signal) => await new Promise((_resolve, reject) => {
...
`,
  ...
  it("reports unawaited rejected host calls as failures", async () => {
    const result = await execute(
      "void pi.read({ path: 'missing' }); return 'premature';",
      async () => { throw new Error("missing file"); },
    );
    expect(result.outcome).toBe("failed");
...
`,
  ...
  it("discovers and invokes registered extension tools inside the program", async () => {
    ...
    reloadedRoot.extensions = [{ tools: new Map([
    ...
    childRunner.extensions = [{ tools: new Map([["late_child_only", { definition: { ...echo, name: "late_child_only" } }]]) }];
    ...
    const component = renderExecResult(
      {
        ...
        details: {
          activity: {
            ...
            calls: [
              { sequence: 0, ref: "agents.run", args: { task: "inspect runtime" }, status: "running", activity: "thinking" },
