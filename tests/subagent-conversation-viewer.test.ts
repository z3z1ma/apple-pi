...
const { ConversationViewer } = await import("../components/subagents/src/ui/conversation-viewer.js");
...
describe("ConversationViewer", () => {
	describe("render width safety", () => {
		...
		it("no line exceeds width with embedded ANSI escape codes in content", () => {
			...
			const messages = [{ role: "toolResult", toolUseId: "t1", content: [{ type: "text", text: ansiText }] }];
			for (const w of widths) {
				const viewer = new ConversationViewer(
					mockTui(30, w),
					mockSession(messages),
	...
	describe("safety net against upstream wrapTextWithAnsi bugs", () => {
		...
		it("clamps overwidth lines from toolResult content", () => {
			...
			const messages = [{ role: "toolResult", toolUseId: "t1", content: [{ type: "text", text: "output" }] }];
			const viewer = new ConversationViewer(
				mockTui(30, w),
				mockSession(messages),
				mockRecord(),
	...
	describe("steer composer", () => {
		...
		function makeViewer(opts: { status?: AgentRecord["status"]; onSteer?: (m: string) => void } = {}) {
			...
			const viewer = new ConversationViewer(
				tui,
				mockSession(),
				mockRecord({ status: opts.status ?? "running" }),
				undefined,
				ansiTheme(),
				vi.fn(),
				undefined,
