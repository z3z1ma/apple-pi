...
- **Interactive child session** — is a real Pi session with its own context and persistence. It does not discover package extensions. Mutation-capable children load Codex fast mode, the proactive overflow guard, the home search guard, ledger, wiki, `search_session`, and MCP via explicit paths (`--no-extensions` plus `-e`); public read-only roles load the safety guards, wiki, and `search_session`. Either may load the pair programmer sidecar when `pair: true`. The internal `/btw` child loads only Codex fast mode and the mandatory safety guards. A child may inherit skills unless `isolated`. It must not create another top-level subagent manager or gain `pi_exec` as a way around nested-delegation limits.
- **`pi_exec` guest/worker** — runs disposable JavaScript with an explicit bridge to selected Pi tools, captured extension tools, fetch, and model workers. It does not receive ambient Node filesystem or process authority. Nested model workers receive only explicitly granted core tools and bound context. They load Codex fast mode, the proactive overflow guard, the home search guard, ledger, wiki, and `search_session` extensions the same `--no-extensions` plus `-e` way, and they do not load `pi_exec`, the subagent manager, or MCP.
