...
- **Interactive child session** — is a real Pi session with its own context and persistence. It does not discover package extensions. Ordinary children load the proactive overflow guard, ledger, `session_search`, and MCP via explicit paths (`--no-extensions` plus `-e`), and may load the Advisor sidecar when `advisor: true`; the internal `/btw` child loads only the mandatory overflow guard. A child may inherit skills unless `isolated`. It must not create another top-level subagent manager or gain `pi_exec` as a way around nested-delegation limits.
- **`pi_exec` guest/worker** — runs disposable JavaScript with an explicit bridge to selected Pi tools, captured extension tools, fetch, and model workers. It does not receive ambient Node filesystem or process authority. Nested model workers receive only explicitly granted core tools and bound context. They load the proactive overflow guard, ledger, and `session_search` extensions the same `--no-extensions` plus `-e` way, and they do not load `pi_exec`, the subagent manager, or MCP.
...
- Child sessions and `pi_exec` workers do not discover package extensions. Ordinary children load the proactive overflow guard, ledger, `session_search`, and MCP via explicit paths under `--no-extensions`; `advisor: true` adds the Advisor sidecar. The internal `/btw` child loads only the mandatory overflow guard. Workers load the overflow guard, ledger, and `session_search` explicitly. Recursion is prevented because none load `pi_exec` or the top-level subagent manager. Observational memory stays on the root context extension.
