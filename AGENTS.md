...
| `components/input-editor/` | Custom left-rail editor with model metadata and right-justified status (pair review, MCP count, context %), replacing Pi's default footer with a zero-line component | Root TUI sessions only. |
...
| `components/session-search/` | Transcript history search (`search_session`) | Search only. Compaction is xAI server-side or Pi's default summarizer, plus the pair programmer's post-compaction notebook packet. |
| `components/notebook/` | Jointly curated working conclusions and exact source recall | Both programmers update sourced conclusions through one validated append path. Live context reflects the current branch; historical observations remain recallable but are not injected. |
...
- Child sessions and `pi_exec` workers do not discover package extensions. Mutation-capable children load vroom (fast mode), automatic-compaction failure safety, the search root guard, ledger, wiki, `search_session`, and MCP via explicit paths under `--no-extensions`; public read-only roles load the safety guards, wiki, and `search_session`. `pair: true` adds the pair programmer sidecar. The internal `/btw` child loads only vroom (fast mode) and the mandatory safety guards. Workers load vroom (fast mode), both safety guards, ledger, wiki, and `search_session` explicitly. Recursion is prevented because none load `pi_exec` or the top-level subagent manager. Notebook maintenance stays with the root pair programmer integration.
- Public `agent` children receive a child-only `clarify` tool that consults an independent, ephemeral, read-only snapshot of their immediate parent. It is available without context inheritance or nested-delegation permission. Root sessions, Pi Exec workers, and internal sessions do not receive it. Each fork owns its session state and follows the calling child's cancellation; answers are advice, not new authorization.
...
- wiki behavior: keep derived Markdown graph logic and compact prompt/tool registration in `components/wiki/` plus `extensions/wiki.ts`, mutation procedures in `skills/llm-wiki/`, and product semantics in `docs/wiki.md`. Do not add a stored graph, cache, watcher, registry, or wiki mutation tool without a concrete new failure.
...
Never move credentials, private transcript content, or notebook records into repository fixtures, logs, package contents, or third-party calls. Tests should redirect runtime state to temporary directories rather than touching a developer's real Pi configuration.
