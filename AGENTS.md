...
- There is **one compaction hook owner**. On xAI Responses models that is server-side `/responses/compact`; otherwise Pi's default summarizer runs. The notebook does not register a compact hook; it refreshes one packet of current conclusions before each model request, independently of compaction.
